...
export default function Button({ children, onClick, type }) {
  ...
  if (type === 'checkout' || type === 'login')
    return (
      <button
        onClick={onClick}
        className=' gap-3 relative p-4 bg-Green text-White font-semibold rounded-md shadow-md border-2 border-DarkGreen flex items-center  justify-center overflow-hidden active:bg-DarkGreen uppercase '
      >
        {children}
      </button>
    );
